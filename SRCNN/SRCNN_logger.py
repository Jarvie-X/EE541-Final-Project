...
class Logger:
    ...
    def append(self, loss, train_snr, valid_snr, train_psnr, valid_psnr, train_ssim, valid_ssim):
        ...
        with open(filename, 'w', newline='') as csvfile:
            ...
            for i in range(len(self.loss_values)):
                writer.writerow({'Epoch': i + 1,
                                 'Content Loss': self.loss_values[i],
...
def plot(logger):
    ...
    plt.suptitle("SRCNN(5-3-5) training criterions")
    plt.tight_layout()
    plt.savefig("result_plot_535.png")
    plt.show()
