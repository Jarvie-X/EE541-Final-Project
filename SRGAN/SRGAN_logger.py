...
class Logger:
    ...
    def append(self, content_loss, adversarial_loss, train_snr, valid_snr, train_psnr, valid_psnr, train_ssim, valid_ssim):
    ...
    def save_to_csv(self, filename):
        with open(filename, 'w', newline='') as csvfile:
            fieldnames = ['Epoch', 'Content Loss', 'Adversarial Loss',
                          'Train SNR', 'Valid SNR', 
                          'Train PSNR', 'Valid PSNR', 
                          'Train SSIM', 'Valid SSIM']
...
def plot(logger):
    ...
    plt.subplot(2, 2, 1)
    plt.plot(epochs, logger.content_loss_values, label="Loss")
    ...
    plt.suptitle("SRGAN training criterions")
    plt.tight_layout()
    plt.savefig("result_plot_SRGAN.png")
    plt.show()
